...
function imgui.OnDrawFrame()
	...
	if win_state['update'].v then -- окно обновления скрипта
		...
		if imgui.Button(u8'Скачать и установить обновление', btn_size) then
			async_http_request('GET', 'https://raw.githubusercontent.com/KabanBunya/Mono-Tools/main/Monopoly.lua', nil, 
				function(response) -- вызовется при успешном выполнении и получении ответа
				local f = assert(io.open(getWorkingDirectory() .. '/Monopoly.lua', 'wb'))
				f:write(response.text)
				f:close()
				sampAddChatMessage("[Mono Tools]{FFFFFF} Обновление успешно, перезагружаем скрипт.", 0x046D63)
				thisScript():reload()
			end,
			function(err) -- вызовется при ошибке, err - текст ошибки. эту функцию можно не указывать
				print(err)
				sampAddChatMessage("[Mono Tools]{FFFFFF} Произошла ошибка при обновлении, попробуйте позже.", 0x046D63)
...
function update() -- проверка обновлений
	local zapros = https.request("https://raw.githubusercontent.com/KabanBunya/Monopoly/main/monopoly.json")
	...
	if zapros ~= nil then
		...
		if info2.latest_number ~= nil and info2.latest ~= nil and info2.drop ~= nil then
			...
			if version > tonumber(thisScript().version_num) then
				sampAddChatMessage("[Mono Tools]{FFFFFF} Обнаружено обновление до версии "..updatever..".", 0x046D63)
				win_state['update'].v = true
				UpdateNahuy = true
			else
				if checkupd then
					sampAddChatMessage("[Mono Tools]{FFFFFF} У вас стоит актуальная версия скрипта: "..thisScript().version..".", 0x046D63)
					checkupd = false
				end
				UpdateNahuy = true
			end
	else
		sampAddChatMessage("[Mono Tools]{FFFFFF} Не удалось проверить наличие обновлений, попробуйте позже.", 0x046D63)
		UpdateNahuy = true
	end
end
end
...
function load_settings() -- загрузка настроек
	...
	lovec = imgui.ImBool(ini.settings.lovec)
